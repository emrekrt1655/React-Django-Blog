            ...
            return list.push(
                <div className="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                    <div className="col p-4 d-flex flex-column position-static">
                        <strong className="d-inline-block mb-2 text-primary"> {capitalizeFirstLetter(post.category)} </strong>
                        <h3 className="mb-0"> {post.title} </h3>
                        <div className="mb-1 text-muted"> {post.month} {post.day} </div>
                        <p className="card-text mb-auto"> {post.excerpt} </p>
                        <Link to={`/blog/${post.slug}`} className="stretched-link">Continue reading</Link>
                    </div>
